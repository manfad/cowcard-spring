...
import type { Semen } from "@/lib/types";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";

export const Route = createFileRoute("/_auth/semen/")({ component: SemenPage });
...
function SemenPage() {
  ...
  const { data: semen = [], isLoading } = useQuery({
    queryKey: ["semen"],
    queryFn: async () => { const res = await semenApi.getAll(); return res.data.data ?? []; },
  });

  const columns = [
    columnHelper.accessor("name", { header: "Name" }),
    columnHelper.accessor("sireId", { header: "Sire ID", cell: (info) => info.getValue() ?? "-" }),
    columnHelper.accessor("date", { header: "Date", cell: (info) => new Date(info.getValue()).toLocaleDateString() }),
    columnHelper.accessor("straw", { header: "Straw", cell: (info) => info.getValue() ?? "-" }),
    columnHelper.accessor("bull", {
      header: "Bull",
      cell: (info) => info.getValue() != null ? (<Badge variant={info.getValue() ? "default" : "outline"}>{info.getValue() ? "Yes" : "No"}</Badge>) : "-",
    }),
    columnHelper.accessor("remark", { header: "Remark", cell: (info) => info.getValue() ?? "-" }),
  ];

  const table = useReactTable({
    data: semen, columns, getCoreRowModel: getCoreRowModel(), getFilteredRowModel: getFilteredRowModel(),
  ...
  return (
    <div className="space-y-4">
      ...
      <Input placeholder="Search semen..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} className="max-w-sm" />
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            ...
            : (<TableRow><TableCell colSpan={columns.length} className="h-24 text-center">No semen records found.</TableCell></TableRow>)}
          </TableBody>
        </Table>
      </div>
