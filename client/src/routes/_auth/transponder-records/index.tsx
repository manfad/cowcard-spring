...
import { transponderRecordApi } from "@/lib/api";
import type { TransponderRecord } from "@/lib/types";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";

export const Route = createFileRoute("/_auth/transponder-records/")({ component: TransponderRecordsPage });

const columnHelper = createColumnHelper<TransponderRecord>();

function TransponderRecordsPage() {
  const [globalFilter, setGlobalFilter] = useState("");

  const { data: records = [], isLoading } = useQuery({
    queryKey: ["transponder-records"],
    queryFn: async () => { const res = await transponderRecordApi.getAll(); return res.data.data ?? []; },
  });
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-3xl font-bold">Transponder Records</h1>
      <Input placeholder="Search records..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} className="max-w-sm" />
