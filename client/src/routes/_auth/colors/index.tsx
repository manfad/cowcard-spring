...
export const Route = createFileRoute("/_auth/colors/")({ component: ColorsPage });
...
function ColorsPage() {
  ...
  const { data: colors = [], isLoading } = useQuery({
    queryKey: ["colors"],
    queryFn: async () => { const res = await colorApi.getAll(); return res.data.data ?? []; },
  });
  ...
  const table = useReactTable({
    data: colors, columns, getCoreRowModel: getCoreRowModel(), getFilteredRowModel: getFilteredRowModel(),
  ...
  return (
    <div className="space-y-4">
      ...
      <Input placeholder="Search colors..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} className="max-w-sm" />
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            ...
            : (<TableRow><TableCell colSpan={columns.length} className="h-24 text-center">No colors found.</TableCell></TableRow>)}
          </TableBody>
        </Table>
      </div>
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{table.getFilteredRowModel().rows.length} color(s) total</p>
