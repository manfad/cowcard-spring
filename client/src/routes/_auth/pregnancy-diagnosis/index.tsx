...
export const Route = createFileRoute("/_auth/pregnancy-diagnosis/")({ component: PregnancyDiagnosisPage });
...
function PregnancyDiagnosisPage() {
  ...
  const { data: records = [], isLoading } = useQuery({
    ...
    queryFn: async () => { const res = await pregnancyDiagnosisApi.getAll(); return res.data.data ?? []; },
  });
  ...
  const columns = [
    ...
    columnHelper.accessor("diagnosisBy", { header: "Diagnosis By", cell: (info) => info.getValue()?.name ?? "-" }),
    columnHelper.accessor("pdStatus", { header: "PD Status", cell: (info) => info.getValue()?.name ?? "-" }),
  ];
