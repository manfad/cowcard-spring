...
import { cowRoleApi } from "@/lib/api";
import type { CowRole } from "@/lib/types";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";

export const Route = createFileRoute("/_auth/cow-roles/")({ component: CowRolesPage });
...
function CowRolesPage() {
  ...
  const { data: roles = [], isLoading } = useQuery({
    queryKey: ["cow-roles"],
    queryFn: async () => { const res = await cowRoleApi.getAll(); return res.data.data ?? []; },
  });
  ...
  const table = useReactTable({
    data: roles, columns, getCoreRowModel: getCoreRowModel(), getFilteredRowModel: getFilteredRowModel(),
  ...
  return (
    <div className="space-y-4">
      ...
      <Input placeholder="Search roles..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} className="max-w-sm" />
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{table.getFilteredRowModel().rows.length} role(s) total</p>
